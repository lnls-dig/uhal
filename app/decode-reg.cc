...
static void try_unsigned(
    unsigned &dest, const argparse::ArgumentParser &args, const char *flag)
{
...
int main(int argc, char *argv[])
{
    ...
    if (argc < 2) {
        fputs("Usage: decode-reg mode <mode specific options>\n\n"
              ...
              "mode      mode of operation ('reset', 'build_info', 'sdb', "
              "'decode', 'ram', 'acq', 'lamp', 'pos_calc', 'si57x', "
              "'fmc_active_clk', 'fmc250m_4ch', 'spi')\n",
            stderr);
        return 1;
    }
    std::string mode = argv[1];

    argparse::ArgumentParser parent_args(
        "decode-reg", "1.0", argparse::default_arguments::none);
    ...
    parent_args.add_argument("-a")
        .help("enumerated position of device")
        .required()
        .scan<'u', unsigned>()
        .default_value((unsigned)0);
    parent_args.add_argument("-v")
        .help("verbose output")
        .default_value(false)
        .implicit_value(true);

    argparse::ArgumentParser parent_args_with_help(
        "decode-reg <action>", "1.0", argparse::default_arguments::help);
    parent_args_with_help.add_parents(parent_args);

    argparse::ArgumentParser build_info_args(
        "decode-reg build_info", "1.0", argparse::default_arguments::help);
    build_info_args.add_parents(parent_args);
    build_info_args.add_argument("-q")
        .help("quiet: print only main synthesis name")
        .default_value(false)
        .implicit_value(true);

    argparse::ArgumentParser decode_args(
        "decode-reg decode", "1.0", argparse::default_arguments::help);
    ...
    decode_args.add_argument("-w")
        .help("watch registers")
        .default_value(false)
        .implicit_value(true);
    decode_args.add_argument("-t")
        .help("time register reading")
        .default_value(false)
        .implicit_value(true);
    decode_args.add_argument("-B")
        .help("binary dump")
        .default_value(false)
        .implicit_value(true);

    argparse::ArgumentParser ram_args(
        "decode-reg ram", "1.0", argparse::default_arguments::help);
    ram_args.add_parents(parent_args);
    ram_args.add_argument("-f")
        .help("first address")
        .required()
        .scan<'x', unsigned>();
    ram_args.add_argument("-l")
        .help("last address")
        .required()
        .scan<'x', unsigned>();

    argparse::ArgumentParser acq_args(
        "decode-reg acq", "1.0", argparse::default_arguments::help);
    acq_args.add_parents(parent_args);
    acq_args.add_argument("-c")
        .help("channel number")
        .required()
        .scan<'u', unsigned>();
    acq_args.add_argument("-n")
        .help("number of pre samples")
        .required()
        .scan<'u', unsigned>();
    acq_args.add_argument("-p")
        .help("number of post samples")
        .scan<'u', unsigned>();
    acq_args.add_argument("-s").help("number of shots").scan<'u', unsigned>();
    acq_args.add_argument("-t").help("trigger type");
    acq_args.add_argument("-e")
        .help("data trigger threshold")
        .scan<'u', unsigned>();
    acq_args.add_argument("-l")
        .help("use negative edge data trigger")
        .default_value(false)
        .implicit_value(true);
    acq_args.add_argument("-z")
        .help("data trigger selection")
        .scan<'u', unsigned>();
    acq_args.add_argument("-i")
        .help("data trigger filter")
        .scan<'u', unsigned>();
    acq_args.add_argument("-C")
        .help("data trigger channel")
        .scan<'u', unsigned>();
    acq_args.add_argument("-d").help("trigger delay").scan<'u', unsigned>();

    argparse::ArgumentParser lamp_args(
        "decode-reg lamp", "1.0", argparse::default_arguments::help);
    lamp_args.add_parents(parent_args);
    lamp_args.add_argument("-e")
        .help("enable amplifier for channel")
        .default_value(false)
        .implicit_value(true);
    lamp_args.add_argument("-c")
        .help("channel number")
        .required()
        .scan<'u', unsigned>();
    lamp_args.add_argument("-m")
        .help("choose output mode. use 'help' to see available options")
        .required();
    ...
    argparse::ArgumentParser si57x_args(
        "decode-reg si57x", "1.0", argparse::default_arguments::help);
    si57x_args.add_parents(parent_args);
    si57x_args.add_argument("-s")
        .help("startup frequency")
        .required()
        .scan<'f', double>();
    si57x_args.add_argument("-f")
        .help("desired frequency")
        .required()
        .scan<'f', double>();

    argparse::ArgumentParser spi_args(
        "decode-reg spi", "1.0", argparse::default_arguments::help);
    spi_args.add_parents(parent_args);
    spi_args.add_argument("-q").help("type of SPI device").required();
    spi_args.add_argument("-r")
        .help("register address")
        .scan<'x', uint8_t>()
        .required();
    spi_args.add_argument("-w").help("register value").scan<'x', uint8_t>();
    spi_args.add_argument("-d")
        .help("set default parameters")
        .default_value(false)
        .implicit_value(true);
    spi_args.add_argument("-s")
        .help("slave select")
        .default_value((unsigned)0)
        .scan<'d', unsigned>()
        .required();
    ...
    if (mode == "reset" || mode == "sdb" || mode == "pos_calc"
        || mode == "fmc_active_clk" || mode == "fmc250m_4ch") {
    ...
    if (auto v = args.present<std::string>("--slot"))
        dev_open_slot(bars, v->c_str());
    else if (auto v = args.present<std::string>("--address"))
        dev_open(bars, v->c_str());
    else if (auto v = args.present<std::string>("--serial"))
        dev_open_serial(bars, v->c_str());
    else {
    ...
    if (mode == "acq") {
        ...
        if (auto v = args.present<std::string>("-t"))
            ctl.trigger_type = *v;
    ...
    if (mode == "lamp") {
        ...
        ctl.write_channel("MODE", channel,
            (int32_t)get_index(args.get<std::string>("-m"), lamp::mode_list));
        ...
        ctl.write_channel(
            "TRIG_EN", channel, (int32_t)args.get<unsigned>("-T"));
    ...
    if (mode == "pos_calc") {
        ...
        if (auto v = read_sdb(&bars, dec.match_devinfo_lambda, dev_index)) {
            ...
            fprintf(
                stderr, "Couldn't find pos_calc module index %u\n", dev_index);
            return 1;
        }
    ...
    if (mode == "spi") {
        auto spi_fn = [&bars, &args, dev_index](auto &ctl, const auto &name) {
            ...
            if (auto v = read_sdb(
                    &bars,
                    [&ctl](auto const &d) { return ctl.match_devinfo(d); },
                    dev_index)) {
                ctl.set_devinfo(*v);
            } else {
                fprintf(stderr, "Couldn't find %s module index %u\n",
