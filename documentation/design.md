...
mechanism to determine what region of the underlying memory that the BAR is
actually pointing at; this is done by writing into the paging registers located
in `BAR0`.
...
software must explicitly do by configuring the addresses each acquisition core
can write into.
...
The register maps for each FPGA cores are defined using
