...
Core::Core(struct pcie_bars &bars)
    : RegisterDecoder(bars, ref_devinfo,
          {
              PRINTER("STRP_COMPLETE", "Startup registers status", "invalid",
                  "valid"),
              PRINTER("CFG_IN_SYNC", "Registers synchronization status",
                  "values NOT in sync", "values in sync"),
              PRINTER("I2C_ERR", "I2C error status", "No errors",
                  "I2C error ocurred"),
              PRINTER("BUSY", "Controller busy status", PrinterType::boolean),
              PRINTER("RFREQ_MSB_STRP",
              ...
              PRINTER("RFREQ_LSB_STRP",
                  "RFREQ startup value (least significant bits)",
                  PrinterType::value_hex),
              PRINTER("RFREQ_MSB", "RFREQ value (most significant bits)",
                  PrinterType::value_hex),
              PRINTER("N1", "N1 value", PrinterType::value),
              PRINTER("HSDIV", "HSDIV value", PrinterType::value),
              PRINTER("RFREQ_LSB", "RFREQ value (least significant bits)",
                  PrinterType::value_hex),
          })
...
void Core::decode()
{
    add_general("READ_STRP_REGS",
        rf_get_bit(regs.ctl, WB_SI57X_CTRL_REGS_CTL_READ_STRP_REGS));
    add_general(
        "APPLY_CFG", rf_get_bit(regs.ctl, WB_SI57X_CTRL_REGS_CTL_APPLY_CFG));

    add_general("STRP_COMPLETE",
        get_bit(regs.sta, WB_SI57X_CTRL_REGS_STA_STRP_COMPLETE));
    add_general(
        "CFG_IN_SYNC", get_bit(regs.sta, WB_SI57X_CTRL_REGS_STA_CFG_IN_SYNC));
    ...
    add_general("RFREQ_MSB_STRP",
        extract_value(regs.hsdiv_n1_rfreq_msb_strp,
            WB_SI57X_CTRL_REGS_HSDIV_N1_RFREQ_MSB_STRP_RFREQ_MSB_STRP_MASK));
    add_general("N1_STRP",
        extract_value(regs.hsdiv_n1_rfreq_msb_strp,
            WB_SI57X_CTRL_REGS_HSDIV_N1_RFREQ_MSB_STRP_N1_STRP_MASK));
    add_general("HSDIV_STRP",
        extract_value(regs.hsdiv_n1_rfreq_msb_strp,
            WB_SI57X_CTRL_REGS_HSDIV_N1_RFREQ_MSB_STRP_HSDIV_STRP_MASK));

    add_general("RFREQ_LSB_STRP", regs.rfreq_lsb_strp);

    add_general("RFREQ_MSB",
        rf_extract_value(regs.hsdiv_n1_rfreq_msb,
            WB_SI57X_CTRL_REGS_HSDIV_N1_RFREQ_MSB_RFREQ_MSB_MASK, false));
    add_general("N1",
        rf_extract_value(regs.hsdiv_n1_rfreq_msb,
            WB_SI57X_CTRL_REGS_HSDIV_N1_RFREQ_MSB_N1_MASK, false));
    add_general("HSDIV",
        rf_extract_value(regs.hsdiv_n1_rfreq_msb,
            WB_SI57X_CTRL_REGS_HSDIV_N1_RFREQ_MSB_HSDIV_MASK, false));
