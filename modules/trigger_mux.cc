...
Core::Core(struct pcie_bars &bars)
    : RegisterDecoder(bars, ref_devinfo,
          {
              PRINTER(
                  "RCV_SRC", "Receiver Source", "Triggers", "Internal Signals"),
              PRINTER(
                  "RCV_IN_SEL", "Select Receiver Input", PrinterType::value),
              PRINTER("TRANSM_SRC", "Transmitter Source", "Triggers",
                  "Internal Signals"),
              PRINTER("TRANSM_OUT_SEL", "Select Transmitter Output",
                  PrinterType::value),
          })
...
void Core::decode()
{
    ...
    for (unsigned i = 0; i < *number_of_channels; i++) {
        ...
        add_channel("RCV_IN_SEL", i,
            extract_value<uint32_t>(t, WB_TRIG_MUX_CH0_CTL_RCV_IN_SEL_MASK));
        add_channel("TRANSM_SRC", i, t & WB_TRIG_MUX_CH0_CTL_TRANSM_SRC);
        add_channel("TRANSM_OUT_SEL", i,
            extract_value<uint32_t>(
                t, WB_TRIG_MUX_CH0_CTL_TRANSM_OUT_SEL_MASK));
    }
}
...
void Controller::encode_params()
{
    ...
    for (unsigned i = 0; i < internal::number_of_channels; i++) {
        auto insert_val = [this, i](auto value, uint32_t mask) {
            if (value)
                clear_and_insert(regs.ch[i].ctl, *value, mask);
        };
        auto insert_bit_val = [this, i](auto value, uint32_t mask) {
            if (value)
                insert_bit(regs.ch[i].ctl, *value, mask);
        };

        insert_bit_val(parameters[i].rcv_src, WB_TRIG_MUX_CH0_CTL_RCV_SRC);
        insert_val(
            parameters[i].rcv_in_sel, WB_TRIG_MUX_CH0_CTL_RCV_IN_SEL_MASK);
        insert_bit_val(
            parameters[i].transm_src, WB_TRIG_MUX_CH0_CTL_TRANSM_SRC);
        insert_val(parameters[i].transm_out_sel,
            WB_TRIG_MUX_CH0_CTL_TRANSM_OUT_SEL_MASK);
    }
}
