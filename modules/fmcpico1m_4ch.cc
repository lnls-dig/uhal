...
static_assert(
    WB_FMCPICO1M_4CH_CSR_REG_DATA2 == offsetof(fmcpico1m_4ch, data[2]));
    ...
    : RegisterDecoder(bars, ref_devinfo,
          {
              ...
              PRINTER("RANGE", "Input Range Control for ADC",
                  range_list[0].c_str(), range_list[1].c_str()),
...
{
    add_general("PRSNT",
        get_bit(regs.fmc_status, WB_FMCPICO1M_4CH_CSR_FMC_STATUS_PRSNT));

    add_channel(
        "RANGE", 0, rf_get_bit(regs.rng_ctl, WB_FMCPICO1M_4CH_CSR_RNG_CTL_R0));
    add_channel(
        "RANGE", 1, rf_get_bit(regs.rng_ctl, WB_FMCPICO1M_4CH_CSR_RNG_CTL_R1));
    add_channel(
        "RANGE", 2, rf_get_bit(regs.rng_ctl, WB_FMCPICO1M_4CH_CSR_RNG_CTL_R2));
    add_channel(
        "RANGE", 3, rf_get_bit(regs.rng_ctl, WB_FMCPICO1M_4CH_CSR_RNG_CTL_R3));
