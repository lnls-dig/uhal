...
namespace {
    ...
    uint64_t val2hw_rfreq(double rfreq)
    {
        return rfreq * rfreq_factor;
    }
}
...
bool si57x_parameters::set_freq(double freq)
{
    ...
    for (auto hs_div_opt: hs_div_opts) {
        for (uint32_t n1 = n1_min_val; n1 <= n1_max_val; n1 += n1_step) {
            double rfreq_val = (freq * hs_div_opt * n1 / fxtal);
            double fdco = rfreq_val * fxtal;
