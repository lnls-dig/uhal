...
size_t get_index(
    std::string_view value, const std::vector<std::string> &value_list)
{
...
uint32_t float2fixed(double v, unsigned point_pos, bool saturate)
{
    ...
    if (saturate) {
        if (v >= max_value)
            return max_value_rep;
        if (v <= min_value)
            return min_value_rep;
    } else {
        if (v > max_value) {
            ...
            throw std::runtime_error(
                std::string("value greater than max representable value of ")
                + std::to_string(max_value));
        }
        if (v < min_value) {
            throw std::runtime_error(
                std::string("value less than min representable value of ")
