...
{
    const unsigned point_pos = 31;
    std::tuple<double, bool> values[] = { { .5, false },
        { 1, /* this case is always allowed to saturate */ false },
        { 1.1, true }, { 157, true }, { -1, false }, { -1.005, true },
        { -8, true } };
    for (auto &&[v, t] : values) {
        if (t) {
            CHECK_THROWS_AS(
                float2fixed(v, point_pos, false), std::runtime_error);
        } else {
