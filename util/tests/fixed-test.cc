...
TEST_CASE("Roundtrip conversions - no integer part", "[fixed]")
{
    ...
    for (auto v: {.5, .25, .125, .0625, -.5, -1.})
      REQUIRE(fixed2float(float2fixed(v, point_pos), point_pos) == v);
}
...
TEST_CASE("Saturation", "[fixed]")
{
    ...
    for (auto &&[v, s]: values)
        REQUIRE(fixed2float(float2fixed(v, point_pos, true), point_pos) == s);
}
...
TEST_CASE("Exception with saturation disabled", "[fixed]")
{
    ...
    for (auto &&[v, t]: values) {
        if (t) {
            REQUIRE_THROWS_AS(float2fixed(v, point_pos, false), std::runtime_error);
        } else {
            REQUIRE_NOTHROW(float2fixed(v, point_pos, false));
        }
    }
}
