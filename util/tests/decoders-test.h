...
struct TestRegisterDecoder : public RegisterDecoder {
    ...
    void decode() override
    {
        ...
        add_general("RF_DOUBLE",
            rf_fixed2float(rf_whole_register(regs.fixed_point), 23));
    }
    ...
    void channel_add(unsigned channels, bool extra_channel = false)
    {
        ...
        if (extra_channel)
            add_channel_double("C_DOUBLE", i + 1, 1.);
    }

    /* Channel data single channel & Print */
    void add_ch1_only_double() { add_channel_double("C_DOUBLE_CH1", 1, 1.); }
