...
TEST_CASE("General data storage - bad variant", "[decoders-test]")
{
    ...
    dec.decode();
    CHECK_THROWS_AS(
        dec.get_general_data<double>("INT"), std::bad_variant_access);
    CHECK_THROWS_AS(
        dec.get_general_data<int32_t>("DOUBLE"), std::bad_variant_access);
}
...
TEST_CASE("Channel data storage - bad variant", "[decoders-test]")
{
    ...
    dec.channel_decode();
    CHECK_THROWS_AS(
        dec.get_channel_data<double>("C_INT", 0), std::bad_variant_access);
}
...
TEST_CASE("Channel data number_of_channels", "[decoders-test]")
{
    ...
    CHECK_THROWS_AS(
        dec.get_channel_data<int32_t>("C_INT", 2), std::out_of_range);
}
...
TEST_CASE("Channel data single channel", "[decoders-test]")
{
    ...
    CHECK_THROWS_AS(
        dec.get_channel_data<double>("C_DOUBLE_CH1", 0), std::out_of_range);
}
