...
void dev_open_slot(struct pcie_bars &bars, const char *slot)
{
    char slot_path[64];
    snprintf(
        slot_path, sizeof slot_path, "/sys/bus/pci/slots/%s/address", slot);
    std::ifstream fslot { slot_path };
    if (fslot.fail())
        throw std::runtime_error(
            std::string("couldn't open slot file: ") + slot_path);
...
void dev_open(struct pcie_bars &bars, const char *pci_address)
{
    ...
    const char wc_resource_path_fmt[]
        = "/sys/bus/pci/devices/%s.0/resource%d_wc";
    ...
    for (unsigned i = 0; i < 3; i++) {
        ...
        snprintf(resource_path, sizeof resource_path, wc_resource_path_fmt,
            pci_address, bar);
        int fd = open(resource_path, O_RDWR);
        if (fd < 0) {
            snprintf(resource_path, sizeof resource_path, resource_path_fmt,
                pci_address, bar);
            fd = open(resource_path, O_RDWR);
            if (fd < 0)
                throw std::runtime_error(
                    std::string("couldn't open resource file: ") + pci_address);
        }
        if (bar == 0)
            lock_file(fd);
        struct stat st;
        ...
        if (ptr == MAP_FAILED) {
            throw std::runtime_error(
                std::string("couldn't mmap resource file: ") + pci_address);
        }
        switch (bar) {
        case 0:
            bars.bar0 = ptr;
...
void dev_open_serial(struct pcie_bars &bars, const char *dev_file)
{
    ...
    if (fd < 0)
        throw std::runtime_error(
            std::string("couldn't open serial port: ") + dev_file);

    auto xfail = [](int rv) {
        if (rv < 0)
            throw std::runtime_error(
                std::string("couldn't configure serial port: ")
                + strerror(errno));
    };
